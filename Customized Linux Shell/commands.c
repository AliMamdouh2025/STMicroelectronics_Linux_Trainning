...
/**
 * Print the system's memory (RAM and swap) usage information to the standard output.
 *
 * This function retrieves and displays the total, used, and free memory for both 
 * RAM and swap space in megabytes (MB). The memory information is obtained using 
...
void cmd_free(void) 
{
	...
	// Retrieve system information and check for errors
	if (sysinfo(&info) != EMPTY) 
	{
		// Print an error message if sysinfo fails
		perror("Error in cmd_free: sysinfo");
		return; // Exit the function if there's an error
	}

	// Calculate total RAM in bytes by multiplying with the memory unit size
	long total_ram = info.totalram * info.mem_unit;

	// Calculate free RAM in bytes by multiplying with the memory unit size
	long free_ram = info.freeram * info.mem_unit;

	// Calculate used RAM by subtracting free RAM from total RAM
	long used_ram = total_ram - free_ram;

	// Calculate total swap space in bytes by multiplying with the memory unit size
	long total_swap = info.totalswap * info.mem_unit;

	// Calculate free swap space in bytes by multiplying with the memory unit size
	long free_swap = info.freeswap * info.mem_unit;

	// Calculate used swap space by subtracting free swap from total swap
	long used_swap = total_swap - free_swap;

	// Print the total, used, and free RAM in megabytes (MB)
	printf("RAM:  total: %ld MB, used: %ld MB, free: %ld MB\n", 
			total_ram / (1024 * 1024), used_ram / (1024 * 1024), free_ram / (1024 * 1024));

	// Print the total, used, and free swap space in megabytes (MB)
	printf("Swap: total: %ld MB, used: %ld MB, free: %ld MB\n", 
			total_swap / (1024 * 1024), used_swap / (1024 * 1024), free_swap / (1024 * 1024));
}
...
/**
 * Print the system's uptime and idle time to the standard output.
 *
 * This function retrieves and displays the amount of time the system has been running 
 * since it was last booted (uptime) as well as the total time spent in the idle process. 
 * The uptime is displayed in days, hours, minutes, and seconds, while the idle time is 
 * shown in seconds.
 */
void cmd_uptime(void) 
{
	...
	{
		// Print an error message if sysinfo fails
		perror("Error in cmd_uptime: sysinfo");
		return; // Exit the function if there's an error
	}
	...
	// Calculate the number of days from the uptime
	long days = uptime / (60 * 60 * 24);

	// Calculate the number of hours from the remaining uptime after subtracting days
	long hours = (uptime % (60 * 60 * 24)) / (60 * 60);

	// Calculate the number of minutes from the remaining uptime after subtracting hours
	long minutes = (uptime % (60 * 60)) / 60;

	// Calculate the remaining seconds after subtracting minutes
	long seconds = uptime % 60;

	// Print the system uptime in days, hours, minutes, and seconds
	...
	// Print the total time the system has spent in the idle process in seconds
	// Open the /proc/stat file to read CPU statistics
	FILE *file = fopen("/proc/stat", "r");
	if (file == NULL) 
	{
		// Print an error message if the file cannot be opened
		perror("Error in cmd_uptime: opening /proc/stat");
		return; // Exit the function if there's an error
	}

	// Buffer to store each line read from /proc/stat
	char buffer[1024];
	unsigned long idle_time = 0;

	// Read each line from /proc/stat
	while (fgets(buffer, sizeof(buffer), file)) 
	{
		// Check for the line starting with "cpu " to get the total CPU stats
		if (strncmp(buffer, "cpu ", 4) == STRINGS_ARE_EQUAL) 
		{
			// Variables to hold the CPU times
			unsigned long user, nice, system, idle;

			// Parse the CPU times from the line
...
{
	(void)args; // Unused parameter, Keep it like this for any future work
	print_all_variables(); // we call it from cmd_allVar to achieve abstaraction as possible
}
